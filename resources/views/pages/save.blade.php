            ...
            <div class="p-10 py-5 w-100 max-w-3xl rounded-xl bg-slate-100 rounded-lg overflow-x-auto">
                <table class="m-auto border-collapse bg-slate-100 rounded">
                    ...
                    <thead class="border-2 border-slate-900">
                        <th class="bg-slate-900 color text-white">
                    ...
                    <tbody>
                        @php
                            $isTOPTable = \App\Constants\StadisticsRanges::TOPs->name() == $key;
                        @endphp
                            ...
                            <tr @class(['item', 'h-fit', 'super-top' => ($isTOPTable && $index < $data->tops->superTop), 'top' => ($isTOPTable && $index < $data->tops->top)])>
                                ...
                                <td class="py-1 border-2 border-slate-900">
                                    <div class="mx-auto w-fit">
                                        @if ($row->position > 0)
                                            <i class="m-auto fa-solid fa-arrow-up" style="color: rgb(39, 130, 3);" aria-hidden="true"></i>
                                        @else
                                            @if ($row->position < 0)
                                ...
                                <td class="px-4 py-1 text-center border-2 border-slate-900 arrow"><i class="m-auto fa-solid fa-arrow-right" aria-hidden="true"></i></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
